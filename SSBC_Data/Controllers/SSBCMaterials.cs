...
{
    public static partial class SSBCMaterials
    {
        public static SSBC_Data.SourceContext.MSG Save(SSBC_Data.Entities.SSBCMaterials item)
        {
            ...
            try
            {
                using (var dbContext = new SSBC_Data.SourceContext())
                {
                    ...
                    {
                        var items = dbContext.SSBCMaterials;
                        var itemCheck = items.Where(s => s.MaterialCo == item.MaterialCo).ToList();
        ...
        public static SSBC_Data.SourceContext.MSG Delete(IEnumerable<SSBC_Data.Entities.SSBCMaterials> items)
        {
            var _return = new SSBC_Data.SourceContext.MSG();
            try
            {
                ...
                {
                    var itemDel = dbContext.SSBCMaterials.Where(s => s.MaterialCo == item.MaterialCo).FirstOrDefault();
