...
{
    public static partial class SSBCRedTrackings
    {
        public static SSBC_Data.SourceContext.MSG Save(SSBC_Data.Entities.SSBCRedTrackings item, string Symbol)
        {
            ...
            try
            {
                using (var dbContext = new SSBC_Data.SourceContext())
                {
                    ...
                    {
                        var items = dbContext.SSBCRedTrackings.Where(x => x.ScaleDate.Value.Year == item.ScaleDate.Value.Year);
                        ...
                        else
                        {
                            ...
                            item.TrackNo = "RE" + item.ScaleDate.Value.Year.ToString().Substring(2, 2) + item.Seq.ToString("00000#") + "-" + Symbol;
        ...
        public static SSBC_Data.SourceContext.MSG Delete(IEnumerable<SSBC_Data.Entities.SSBCRedTrackings> items)
        {
            var _return = new SSBC_Data.SourceContext.MSG();
            try
            {
                ...
                {
                    var itemDel = dbContext.SSBCRedTrackings.Where(s => s.TrackNo == item.TrackNo).FirstOrDefault();
