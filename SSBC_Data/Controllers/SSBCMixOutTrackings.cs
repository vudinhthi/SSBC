...
{
    public static partial class SSBCMixOutTrackings
    {
        public static SSBC_Data.SourceContext.MSG Save(SSBC_Data.Entities.SSBCMixOutTrackings item, string Symbol)
        {
            ...
            try
            {
                using (var dbContext = new SSBC_Data.SourceContext())
                {
                    ...
                    {
                        var items = dbContext.SSBCMixOutTrackings.Where(x => x.ScaleDate.Value.Year == item.ScaleDate.Value.Year);
                        ...
                        else
                        {
                            ...
                            item.TrackNo = "MO" + item.ScaleDate.Value.Year.ToString().Substring(2, 2) + item.Seq.ToString("00000#") + "-" + Symbol;
                    ...
                    else if (item.UpdateStatus == "Modified")
                    {
                        ...
                        dbContext.Entry(item).Property(x => x.ScaleDate).IsModified = false;
        ...
        public static SSBC_Data.SourceContext.MSG Delete(IEnumerable<SSBC_Data.Entities.SSBCMixOutTrackings> items)
        {
            var _return = new SSBC_Data.SourceContext.MSG();
            try
            {
                ...
                {
                    var itemDel = dbContext.SSBCMixOutTrackings.Where(s => s.TrackNo == item.TrackNo).FirstOrDefault();
